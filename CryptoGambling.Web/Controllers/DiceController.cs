...
namespace CryptoGambling.Web.Controllers
{
    public class DiceController : Controller
    {
        ...
        public DiceController(IDataManager dataManager, UserManager<UserIdentity> userManager)
        {
        ...
        public async Task<GameOutput?> Bet(BetModel betModel)
        {
            ...
            if (betModel is not null)
            {
                ...
                var game = new Dice(betModel.IntLuckyField, gameInput);
                var res = game.Bet();
                // force to lose
                if (res.IsGameWinning == true && res.Profit > sharedBalance)
                {
                    res.IsGameWinning = false;
                    res.Profit = betModel.BetAmount * -1;
